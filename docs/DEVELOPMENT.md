...
这份文档描述当前实现。datad 是 UFI 下方的设备数据与控制层，不承载 UUID、UFI OTA、插件、数据库或前端业务；唯一例外是 datad 自身的签名 OTA。
...
4. 通过内置 Ed25519 公钥验证 datad 自身更新清单与产物

机型适配继续按“先把已确认模板做实，再逐个补充”的方式推进，不在主路径里堆隐式兼容分支。UUID、UFI OTA、插件、数据库、调度任务和前端鉴权继续留在 UFI；datad 只维护自己的签名更新状态机。
