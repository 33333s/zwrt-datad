...
- `thermal` 是模板规范化后的温度接口，随 `/state` 与 SSE 的 `state` 事件一起发送。`thermal.cpu_celsius` 为模板 CPU 温度；MU5250 的 `thermal.zones` 来自 sysfs thermal zones，已过滤无效哨兵值、按名称排序并转换为摄氏度。其他模板当前返回空 `zones`。新消费者应使用该字段，不再自行解析 `runtime.thermal_zones[].temp_milli`。
